...
public class RMINode3 implements RMIInterface {
  ...
  public static void main(String[] args) {
    RMINode3 node = new RMINode3();
//    node.startAlgorithm("127.0.0.1", 5696, "7");
    node.startAlgorithm("25.31.77.86", 5696, "17000");
  }
  ...
  public void runAlgorithm(String host, Integer port, String id) {
    try {
      ...
      nodeStub.electionMessage(THIS_NODE_ID, Integer.valueOf(THIS_NODE_ID), true);
...
//      executor.scheduleAtFixedRate(this::checkCoordinatorStatus, 0, 10, TimeUnit.SECONDS);
  ...
  @Override
  public void electionMessage(String starterId, Integer winner, boolean firstStep) throws RemoteException {
    boolean ifCurrentForFirst = false;
    if (firstStep) {
      for (String temp : registry.list()) {
        System.out.println("test for first: " + temp + "  " + ifCurrentForFirst);
        if (ifCurrentForFirst) {
          System.out.println("LIST: " + Arrays.toString(registry.list()));
          System.out.println("\t" + winner + "this " + THIS_NODE_ID);
          ...
          ifCurrentForFirst = false;
        }
        ifCurrentForFirst = true;
      }
    } else {
...
//    if (!Objects.equals(starterId, THIS_NODE_ID) || (winner == null && Objects.equals(starterId, THIS_NODE_ID))) {
        ...
        try {
          RMIInterface stub = (RMIInterface) registry.lookup(winner.toString());
          System.out.println("Informing node " + winner + " of this node's victory");
